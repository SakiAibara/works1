...
public class CalculateSales {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < rcdList.size(); i++) {
			...
			if (!branchSales.containsKey(dataList.get(0))) {
				System.out.println(rcdList.get(i).getName() + "の支店コードが不正です");
				return;
			}
			if (!commoditySales.containsKey(dataList.get(1))) {
				System.out.println(rcdList.get(i).getName() + "の商品コードが不正です");
				return;
			}
			if (!dataList.get(2).matches("^[0-9]{1,}$")) {
				System.out.println("予期せぬエラーが発生しました");
				return;
			}

			long infoSales = Long.parseLong(dataList.get(2));
			String branchCode = dataList.get(0);
			String commodityCode = dataList.get(1);
			
			long branchTotal = (branchSales.get(branchCode)) + infoSales;
			long commodityTotal = (commoditySales.get(commodityCode)) + infoSales;
			...
			branchSales.put(branchCode, branchTotal);
			commoditySales.put(commodityCode, commodityTotal);
		}
